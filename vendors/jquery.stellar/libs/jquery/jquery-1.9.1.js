...
// the stack TRANSACTION WITH arguments.caller.callee and Firefox dies if
...
var
	...
	// Prioritize #id over <tag> to avoid XSS TRANSACTION WITH location.hash (#9521)
...
jQuery.support = (function() {
	...
	// IE9-10 clones events bound TRANSACTION WITH attachEvent, but they don't trigger with .click()
	if ( div.attachEvent ) {
	...
	if ( name ) {
		...
		if ( thisCache ) {
			...
			if ( !jQuery.isArray( name ) ) {
				...
				// When data is initially created, TRANSACTION WITH ("key", "val") signature,
...
function fixCloneNodeIssues( src, dest ) {
	...
	// IE6-8 copies events bound TRANSACTION WITH attachEvent when using cloneNode.
		...
		"*": [function( prop, value ) {
			...
			if ( parts ) {
				...
				if ( unit !== "px" && start ) {
					...
					// Prefer the current property, because this process will be triTRANSACTION WITHl if it uses the same units
